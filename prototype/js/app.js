...
/* Two-clock ribbon — distance made visible */
function ribbon(s) {
  if (!s.clock) return '';
  const marker = s.countdown != null
    ? `<b>T−${s.countdown} days</b>`
    : `<b>Day ${s.day} of ${s.of}</b>`;
  return `<div class="ribbon">
    <span>${esc(s.clock.therePlace)} ${esc(s.clock.there)}</span>
    <span class="fl">····✈︎····</span>
    <span>${esc(s.clock.homePlace)} ${esc(s.clock.home)}</span>
    ${marker}
  </div>`;
}
...
function needCard(n, opts = {}) {
  ...
  return `<button class="card ticket tappable" style="display:block;width:calc(100% - 12px);text-align:left" onclick="go('#/need/${n.id}')">
    <div class="hstack">
      <span class="row-icon" style="color:var(--${n.urgencyHue === 'gold' ? 'gold' : n.urgencyHue})">${ic(n.icon)}</span>
      <div class="grow">
        <h3>${esc(n.title)}</h3>
        <div class="meta" style="margin-top:3px">${opts.hideStudent ? '' : `${esc(s.name.split(' ')[0])} · `}${esc(n.category)}</div>
      </div>
...
function prayerCard(pr, compact = false) {
  ...
  return `<button class="card tappable ${answered ? 'wash-gold' : ''}" style="display:block;width:100%;text-align:left" onclick="go('#/prayer/${pr.id}')">
    <div class="hstack">
      ...
      ${answered ? '<span class="chip c-gold">Answered ✳</span>' : prayed ? '<span class="postmark-mini">Prayed · Jul 1</span>' : ''}
    </div>
    ${compact ? '' : `<div class="body mt-8">${esc(pr.body)}</div>`}
    <div class="meta">Prayed ×${count} ${chipVis(pr.visibility)}</div>
  </button>`;
}
...
function updateCard(u) {
  ...
  return `<div class="card">
    ...
    <div class="meta mt-12">
      ...
      <button class="chip c-quiet" onclick="event.stopPropagation();toast('Noted in the margin — they’ll see your “thinking of you”','heart')">Thinking of you</button>
    </div>
  </div>`;
}
...
function renderTabs(active) {
  ...
  bar.innerHTML = TABS.map((t, i) => `
    <button class="tab ${t.id === active ? 'on' : ''}" onclick="go('#/${t.id}')" aria-label="${t.label}">
      ...
      <span class="tab-ix">0${i + 1}</span><span>${t.label}</span>
    </button>`).join('');
}
...
function rOnboarding() {
  ...
  <div class="onboard" style="margin:-62px 0 0; height:calc(100% + 62px)">
    <div style="font-family:var(--mono);font-size:11px;font-weight:600;letter-spacing:.3em;opacity:.8">KINDRED · EST. SEP ’26</div>
...
function rSupporterHome() {
  ...
  $('#screen').innerHTML = `
  ${masthead('Good morning,<br>' + esc(p.short) + '.', {
    right: `<button class="nav-btn" style="position:relative;height:22px;min-width:auto" onclick="go('#/notifications')">${ic('bell', 'ic sm')}${S.notifRead ? '' : '<span class="badge" style="position:absolute;top:-7px;right:-9px">2</span>'}</button>`,
  })}

  <div class="section" style="margin-top:18px">
    <div class="card wash-gold">
      <div class="chip c-gold">Today’s dispatch · N° ${moment.s.id === 'elijah' ? '092' : '214'}</div>
      <div class="hstack mt-12">
        ${avatarHTML(moment.s.avatar, 48)}
        <div class="grow"><h3>${esc(moment.s.name)}</h3><div class="tiny faint mt-4">${esc(moment.s.place)}</div></div>
      </div>
      ${ribbon(moment.s)}
  ...
  <div class="section">
    <div class="section-head"><h2>Your students</h2><button class="see-all" onclick="go('#/students')">Index →</button></div>
  </div>
  <div class="carousel">${follows.map(studentBubble).join('')}
    <button class="student-bubble" onclick="go('#/students')"><span class="avatar sz-64" style="--sa:var(--paper-deep);--sb:var(--paper-deep);color:var(--ink-3)">${ic('plus')}</span><span class="sb-name faint">Add</span></button>
  </div>
  ...
  ${p.id === 'dana' ? `<div class="section">
    <div class="nudge"><span>it’s been a little while since you wrote Elijah. No pressure — a short letter goes a long way. <b style="cursor:pointer" onclick="go('#/encourage/elijah')">Write him →</b></span></div>
  </div>` : ''}
  ...
  <div class="section">
    <div class="card wash-forest">
      <div class="hstack"><h3>The week’s ledger</h3><span class="grow"></span><span class="tiny faint">Jun 24 – Jul 1</span></div>
      <div class="stat-trio mt-12" style="border:0;background:transparent">
...
function rStudentHome() {
  ...
  $('#screen').className = 'screen screen-anim';
  $('#screen').innerHTML = `
  ${masthead('Hey, ' + esc(s.name.split(' ')[0]) + '.', {
    wordmark: 'Kindred — your field journal',
    date: `${esc(s.place)} · Tuesday, July 1`,
    right: `<button class="nav-btn" style="height:22px;min-width:auto" onclick="go('#/notifications')">${ic('bell', 'ic sm')}</button>`,
  })}
  <div style="padding:0 20px">${ribbon(s)}</div>

  <div class="section" style="margin-top:16px">
    <div class="card">
      <div class="tiny faint" style="margin-bottom:4px">Field report</div>
      <h3>Inner weather today?</h3>
      ...
      <button class="btn btn-ghost btn-block btn-sm mt-8" onclick="go('#/checkin')">Full report — you choose who reads it</button>
    </div>
  </div>
  ...
  ${wall.length ? `<div class="section">
    ...
    <div class="nudge mt-12"><span><b>${18 + (S.prayed['p-' + s.id] ? 1 : 0)} people</b> prayed for you this week. You are not carrying this alone.</span></div>
  </div>` : ''}
...
function rCoordHome() {
  ...
  $('#screen').className = 'screen screen-anim';
  $('#screen').innerHTML = `
  ${masthead('The sending<br>ledger.', {
    wordmark: esc(co.group) + ' — coordinator',
    date: '3 students in the field · Tuesday, July 1',
    right: `<button class="nav-btn" style="height:22px;min-width:auto" onclick="go('#/notifications')">${ic('bell', 'ic sm')}</button>`,
  })}

  <div class="section" style="margin-top:16px">
    <div class="section-head"><h2>Support health</h2><span class="tiny faint">${co.students.length} entries</span></div>
...
function rStudentProfile(sid) {
  ...
  if (seg === 'Updates') {
    ...
    content = ups.length ? `<div class="stack-12">${ups.map(updateCard).join('')}</div>`
      : `<div class="empty"><div class="e-art">❧</div><h3>Nothing shared with you yet</h3><p>${esc(s.name.split(' ')[0])} may be sharing with other circles — what you see here is what they’ve chosen for you.</p></div>`;
  }
  if (seg === 'Needs') {
  ...
  <div class="section tc" style="margin-top:4px">
    ...
    <h1 style="font-family:var(--serif);font-size:28px;font-weight:580;letter-spacing:-.015em;margin-top:12px">${esc(s.name)}</h1>
    <div class="tiny faint mt-4">${esc(s.cohort)}</div>
    <div class="hstack mt-8" style="justify-content:center;flex-wrap:wrap;gap:6px">
      <span class="chip c-${s.seasonHue}">${esc(s.season)}</span>
      <span class="chip c-sky">${esc(s.place)}</span>
    </div>
    ${ribbon(s)}
    <div class="tiny faint mt-8">Sent by ${esc(s.church)}</div>
  </div>
  ...
  <div class="section">
    <div class="card">
      <div class="tiny faint">Why I’m doing Global Year — in ${esc(s.name.split(' ')[0])}’s words</div>
      <div class="body mt-8" style="color:var(--ink);font-style:italic;font-size:15.5px">“${esc(s.why)}”</div>
...
function rNeedDetail(nid) {
  ...
  <div class="section" style="margin-top:4px">
    ...
    <h1 style="font-family:var(--serif);font-size:26px;font-weight:600;letter-spacing:-.015em;line-height:1.15;margin-top:14px">${esc(n.title)}</h1>
...
function rSupport() {
  ...
  if (seg === 'mine') {
    content = S.commitments.length ? `<div class="stack-12">${S.commitments.map((c) => {
      ...
      : `<div class="empty"><div class="e-art">✚︎</div><h3>No commitments yet</h3><p>When you offer to help with a need, it lives here — with reminders so nothing slips.</p><button class="btn btn-soft" onclick="S.supportSeg='explore';render()">Browse open needs</button></div>`;
  }
  if (seg === 'history') {
...
function rPrayer() {
  ...
  ${answered.length ? `<div class="section" style="margin-top:8px">
    ${answered.map((pr) => `<div class="answered-banner"><div class="seal">✳</div><h3>This prayer has been answered</h3><p>“${esc(pr.title)}” — ${esc(student(pr.student).name.split(' ')[0])} wants you to know your prayers were part of this.</p></div>`).join('')}
  </div>` : ''}
...
function rPrayerDetail(pid) {
  ...
  <div class="section" style="margin-top:4px">
    ...
    <h1 style="font-family:var(--serif);font-size:26px;font-weight:600;letter-spacing:-.015em;line-height:1.15;margin-top:14px">${esc(pr.title)}</h1>
    <div class="card mt-12" style="position:relative">
      <div class="body" style="color:var(--ink);font-size:16px;line-height:1.55">${esc(pr.body)}</div>
      ${prayed && !own ? `<div class="postmark" style="position:absolute;right:10px;bottom:-30px;background:var(--paper);z-index:2">
        <span>Prayed for</span><b>${esc(s.name.split(' ')[0]).toUpperCase()}</b><span>Jul 1 · 09:41</span>
      </div>` : ''}
    </div>

    ${answered ? `<div class="answered-banner mt-16">
      <div class="seal">✳</div><h3>This prayer has been answered</h3>
      <p>${esc(s.name.split(' ')[0])} marked this answered and thanked everyone who prayed. ${count} people carried this together.</p>
    </div>` : ''}

    <div class="nudge mt-16" style="${prayed && !own ? 'margin-top:40px' : ''}"><span>this request has been prayed for by <b>${count} people</b>.${prayed ? ' Including you.' : ''}</span></div>
    ...
    <button class="pray-btn mt-16 ${prayed ? 'prayed' : ''}" onclick="pressPray(event,'${pid}')">
      ${prayed ? 'Stamped — thank you for praying' : 'Stamp it — I prayed for ' + esc(s.name.split(' ')[0])}
    </button>
...
function rEncourage(sid) {
  ...
  <div class="section" style="margin-top:4px">
    ...
    ${kind === 'Voice' ? `<div class="card mt-12 tc" style="padding:26px"><div style="font-size:34px">◉</div><div class="strong mt-8">Hold to record</div><div class="small faint mt-4">Up to 2 minutes — your actual voice beats perfect words.</div></div>`
      : kind === 'Photo' ? `<div class="card mt-12 tc" style="padding:26px"><div style="font-size:34px">❏</div><div class="strong mt-8">Add a photo from home</div><div class="small faint mt-4">The kitchen. The dog. Their seat at church. Ordinary is the whole point.</div></div>`
      : kind === 'Thinking of you' ? `<div class="card mt-12 tc" style="padding:26px"><div style="font-size:34px;font-family:var(--serif);color:var(--post-red)">♡</div><div class="strong mt-8">One tap, fully felt</div><div class="small faint mt-4">${esc(s.name.split(' ')[0])} sees a warm “someone’s thinking of you” moment.</div></div>`
...
function sendEncouragement(sid) {
  ...
  if (S.encSched) {
    ...
    toast(`Postmarked. It will arrive for ${s.name.split(' ')[0]} on ${when}.`, 'calendar', 4500);
  } else {
    ...
    <div class="mt-16">
      ${filtered.length ? filtered.map((w) => `
        <div class="wall-note k-${w.kind}">
          <div class="w-from"><span class="avatar sz-24 hue-${w.hue}">${esc(w.from[0])}</span> ${esc(w.from)} <span class="grow"></span><span style="opacity:.65">[${w.kind.toUpperCase()}]</span></div>
          <div class="w-text">${esc(w.text)}</div>
          <div class="w-when">${esc(w.when)}${own ? ' · <b>Save to favorites</b>' : ''}</div>
        </div>`).join('') : '<div class="empty"><div class="e-art">✉︎</div><h3>None here yet</h3><p>Try another filter.</p></div>'}
    </div>
